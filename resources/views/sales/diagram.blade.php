...
@section('content')
<div class="space-y-6">
    <h1 class="font-display text-2xl font-bold tracking-tight text-slate-800">Grafik & Performa Penjualan</h1>

    <!-- Filter -->
    
    <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100 -mt-2">
...
<script>
// Pakai 'load' alih-alih DOMContentLoaded supaya chart nunggu layar 100% siap
window.addEventListener('load', function() {
    
    // Tunda sedikit untuk memastikan transisi layar mulus
    setTimeout(function() {
        ...
        // 🌊 Fungsi Pembuat Jeda Ombak (Makin ke kanan/bawah makin telat)
        const jedaOmbak = (ctx) => {
            if (ctx.type === 'data') {
                return ctx.dataIndex * 150 + (ctx.datasetIndex * 200);
            }
            return 0;
        };

        // 1. ANIMASI NAIK: Khusus Bar Vertikal (Kuning) & Garis -> Tumbuh ke ATAS
        const ANIMASI_NAIK = {
            y: {
                duration: 1200,
                easing: 'easeOutQuart',
        ...
        // 2. ANIMASI SAMPING: Khusus Bar Horizontal (Ungu) -> Tumbuh ke SAMPING KANAN
        const ANIMASI_SAMPING = {
            x: {
                duration: 1200,
                easing: 'easeOutQuart',
                delay: jedaOmbak
        ...
        const tooltipRupiah = {
            callbacks: { label: (ctx) => `${ctx.dataset.label}: ${formatRupiah(ctx.parsed.y ?? ctx.parsed.x ?? ctx.parsed)}` }
        };
        const tooltipAngka = {
            callbacks: { label: (ctx) => `${ctx.dataset.label || ctx.label}: ${new Intl.NumberFormat('id-ID').format(ctx.parsed.y ?? ctx.parsed.x ?? ctx.parsed)}` }
        };
        ...
        @if ($isAdmin)
            ...
            new Chart(document.getElementById('chartTrenAdmin'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Pendapatan', data: Object.values(trenHarian).map(v => v.pendapatan), borderColor: WARNA.pendapatan.line, backgroundColor: WARNA.pendapatan.fill, fill: true, tension: 0.3 },
                        { label: 'Keuntungan', data: Object.values(trenHarian).map(v => v.keuntungan), borderColor: WARNA.keuntungan.line, backgroundColor: WARNA.keuntungan.fill, fill: true, tension: 0.3 }
                    ]
                },
                // Gunakan ANIMASI_NAIK
                options: { animations: ANIMASI_NAIK, plugins: { tooltip: tooltipRupiah }, scales: { y: { beginAtZero: true, ticks: { callback: formatRupiah } } } },
            });

            const topCabang = @json($topCabang);
            new Chart(document.getElementById('chartTopCabang'), {
                type: 'bar',
                data: { labels: topCabang.map(c => c.org_code), datasets: [{ label: 'Jumlah Transaksi', data: topCabang.map(c => c.jumlah_transaksi), backgroundColor: WARNA.cabang, borderRadius: 6 }] },
                // Gunakan ANIMASI_NAIK (Warna Kuning)
                options: { animations: ANIMASI_NAIK, plugins: { tooltip: tooltipAngka }, scales: { y: { beginAtZero: true } } },
            });

            const topCustomerGlobal = @json($topCustomerGlobal);
            new Chart(document.getElementById('chartTopCustomerGlobal'), {
                type: 'bar',
                data: { labels: topCustomerGlobal.map(c => c.nama), datasets: [{ label: 'Pendapatan', data: topCustomerGlobal.map(c => c.pendapatan), backgroundColor: WARNA.customer, borderRadius: 6 }] },
                // Gunakan ANIMASI_SAMPING (Warna Ungu)
                options: { indexAxis: 'y', animations: ANIMASI_SAMPING, plugins: { tooltip: tooltipRupiah }, scales: { x: { beginAtZero: true, ticks: { callback: formatRupiah } } } },
            });

            const komposisi = @json($komposisiProduk);
            new Chart(document.getElementById('chartKomposisi'), {
                type: 'doughnut',
                data: { labels: Object.keys(komposisi), datasets: [{ data: Object.values(komposisi), backgroundColor: WARNA.komposisi }] },
                // Gunakan ANIMASI_MUTER
                options: { animation: ANIMASI_MUTER, plugins: { tooltip: tooltipAngka, legend: { position: 'bottom' } } },
            });
        @else
            const trenTransaksiHarian = @json($trenTransaksiHarian);
            new Chart(document.getElementById('chartTrenKaryawan'), {
                type: 'line',
                data: { labels: Object.keys(trenTransaksiHarian), datasets: [{ label: 'Jumlah Transaksi', data: Object.values(trenTransaksiHarian), borderColor: WARNA.pendapatan.line, backgroundColor: WARNA.pendapatan.fill, fill: true, tension: 0.3 }] },
                options: { animations: ANIMASI_NAIK, plugins: { tooltip: tooltipAngka }, scales: { y: { beginAtZero: true } } },
            });
            ...
            new Chart(document.getElementById('chartTopCustomerCabang'), {
                type: 'bar',
                data: { labels: topCustomerCabang.map(c => c.nama), datasets: [{ label: 'Pendapatan', data: topCustomerCabang.map(c => c.pendapatan), backgroundColor: WARNA.customer, borderRadius: 6 }] },
                options: { indexAxis: 'y', animations: ANIMASI_SAMPING, plugins: { tooltip: tooltipRupiah }, scales: { x: { beginAtZero: true, ticks: { callback: formatRupiah } } } },
            });
            ...
            new Chart(document.getElementById('chartProdukTerlaris'), {
                type: 'doughnut',
                data: { labels: Object.keys(produkTerlaris), datasets: [{ data: Object.values(produkTerlaris), backgroundColor: WARNA.komposisi }] },
                options: { animation: ANIMASI_MUTER, plugins: { tooltip: tooltipAngka, legend: { position: 'bottom' } } },
            });
        @endif
